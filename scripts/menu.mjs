...
async function applyFlow() {
  clear(); banner();
  console.log(c.bold('  📝 Auto-fill application(s)')); hr();
  console.log('');
  console.log(`    ${c.cyan('[1]')} One URL — paste a link, fill that form`);
  console.log(`    ${c.cyan('[2]')} Bulk from roster — use an application_roster.xlsx`);
  console.log(`        (generated by option "e" Process Excel. Each row's URL gets`);
  console.log(`         its matching tailored PDF auto-attached.)`);
  console.log(`    ${c.cyan('[b]')} Back`);
  console.log('');
  console.log(c.yellow('  For ALL modes:'));
  ...
  if (choice === '2') {
    console.log('');
    console.log(c.dim('  Tip: rosters are at output/{ExcelName}_{Date_Time}/application_roster.xlsx'));
    console.log('');
    const rosterPath = await ask(c.bold('  Path to application_roster.xlsx (blank to cancel) › '));
    if (!rosterPath) return;
    if (!fs.existsSync(rosterPath)) { console.log(c.red(`  Not found: ${rosterPath}`)); return pause(); }

    const minStr = await ask(c.bold(`  Minimum score to apply [${c.dim('4.0 — recommended')}] › `)) || '4.0';
    console.log('');
    console.log(c.yellow('  ⚠ This opens one browser per URL. Between each, press Enter to continue.'));
    console.log(c.dim('    For each: review the filled form, click Submit, close the tab → press Enter in terminal.'));
    console.log('');
    const go = (await ask(c.bold('  Proceed? [Y/n] › '))).toLowerCase();
    if (go === 'n') return;

    const bulkArgs = ['scripts/bulk-apply.mjs', '--roster', rosterPath];
    if (parseFloat(minStr) > 0) bulkArgs.push('--min', minStr);
    await run(...bulkArgs);
    return pause();
  }

  console.log(c.red('  Not valid.'));
  return pause();
}
